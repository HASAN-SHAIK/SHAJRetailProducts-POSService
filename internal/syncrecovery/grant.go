...
func Verify(token, publicKeyPEM string) (Grant, error) {
	...
	return Grant{
		RecoveryID: c.RecoveryID, TenantID: c.TenantID, DeviceID: c.DeviceID,
		OrderID: c.OrderID, OrderingKey: c.OrderingKey, EventID: c.EventID,
		ApprovedByUserID: c.ApprovedByUserID, Reason: strings.TrimSpace(c.Reason),
		ExpiresAt: time.Unix(c.Exp, 0).UTC(),
	}, nil
}

func (g Grant) Matches(tenantID, deviceID, orderID, eventID string) bool {
	return g.TenantID == strings.TrimSpace(tenantID) &&
		g.DeviceID == strings.TrimSpace(deviceID) &&
		g.OrderID == strings.TrimSpace(orderID) &&
		g.OrderingKey == "sales_order:"+strings.TrimSpace(orderID) &&
		g.EventID == strings.TrimSpace(eventID)
}
