...
func (s *Service) Apply(ctx context.Context, message Message) error {
	if message.ID == "" || message.Type == "" || !json.Valid(message.Payload) {
		return errors.New("invalid_change_message")
	}
	if message.SchemaVersion == 0 {
		message.SchemaVersion = 1
	}
	if message.Source == "" {
		message.Source = "central"
	}
	...
	return s.db.WithTx(ctx, func(tx *sql.Tx) error {
		...
		if _, err := tx.ExecContext(ctx, `INSERT INTO inbox_messages(message_id,message_type,schema_version,source,payload_json,status,attempt_count,received_at)
			...
			message.ID, message.Type, message.SchemaVersion, message.Source, string(message.Payload), now); err != nil {
			return err
		}
...
func applyTx(ctx context.Context, tx *sql.Tx, m Message) error {
	...
	case "catalog.category.upsert":
		return upsertCategory(ctx, tx, m.Payload)
	case "catalog.categories.snapshot":
		return applyCategorySnapshot(ctx, tx, m.Payload)
	case "catalog.product.upsert":
		return upsertProduct(ctx, tx, m.Payload)
	case "catalog.product.remove":
		return removeProduct(ctx, tx, m.Payload)
	case "catalog.price.upsert":
		return upsertPrice(ctx, tx, m.Payload)
	case "catalog.barcode.upsert":
		return upsertBarcode(ctx, tx, m.Payload)
	case "customer.upsert":
		return upsertCustomer(ctx, tx, m.Payload)
	default:
		return fmt.Errorf("unsupported_change_type:%s", m.Type)
	}
}
...
func upsertCategory(ctx context.Context, tx *sql.Tx, raw []byte) error {
	var p categoryPayload
	if json.Unmarshal(raw, &p) != nil || p.ID == "" || p.Name == "" {
		return errors.New("invalid_category_payload")
	}
	now := time.Now().UTC().Format(time.RFC3339Nano)
	_, err := tx.ExecContext(ctx, `INSERT INTO catalog_categories(id,parent_id,name,code,sort_order,is_active,version,source_updated_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET parent_id=excluded.parent_id,name=excluded.name,code=excluded.code,sort_order=excluded.sort_order,is_active=excluded.is_active,version=excluded.version,source_updated_at=excluded.source_updated_at,updated_at=excluded.updated_at WHERE excluded.version>=catalog_categories.version`, p.ID, p.ParentID, p.Name, p.Code, p.SortOrder, boolInt(p.IsActive), p.Version, p.SourceUpdatedAt, now)
	return err
}
...
type categorySnapshotPayload struct {
	Categories      []categorySnapshotEntry `json:"categories"`
	Version         int                     `json:"version"`
	SourceUpdatedAt *string                 `json:"source_updated_at"`
}
...
func applyCategorySnapshot(ctx context.Context, tx *sql.Tx, raw []byte) error {
	...
	if json.Unmarshal(raw, &p) != nil || p.Version <= 0 {
		return errors.New("invalid_category_snapshot_payload")
	}
	...
	now := time.Now().UTC().Format(time.RFC3339Nano)
	if _, err := tx.ExecContext(ctx, `UPDATE catalog_categories SET is_active=0,version=?,source_updated_at=?,updated_at=? WHERE version<=?`, p.Version, p.SourceUpdatedAt, now, p.Version); err != nil {
		return err
	}
	for _, category := range p.Categories {
		if _, err := tx.ExecContext(ctx, `INSERT INTO catalog_categories(id,parent_id,name,code,sort_order,is_active,version,source_updated_at,updated_at) VALUES(?,NULL,?,NULL,0,1,?,?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name,is_active=1,version=excluded.version,source_updated_at=excluded.source_updated_at,updated_at=excluded.updated_at WHERE excluded.version>=catalog_categories.version`, category.ID, category.Name, p.Version, p.SourceUpdatedAt, now); err != nil {
			return err
		}
	}
	return nil
...
func upsertProduct(ctx context.Context, tx *sql.Tx, raw []byte) error {
	...
	if json.Unmarshal(raw, &p) != nil || p.ID == "" || p.Name == "" {
		return errors.New("invalid_product_payload")
	}
	var gstRateBps any
	if p.GSTRatePercent != nil {
		if *p.GSTRatePercent < 0 || *p.GSTRatePercent > 100 {
			return errors.New("invalid_product_gst_rate")
		}
		gstRateBps = int64(math.Round(*p.GSTRatePercent * 100))
	}
	now := time.Now().UTC().Format(time.RFC3339Nano)
	_, err := tx.ExecContext(ctx, `INSERT INTO catalog_products(id,category_id,sku,name,company,description,unit_of_measure,tax_code,gst_rate_bps,mrp,expiry_date,is_weight_based,is_active,allow_manual_price,track_inventory,version,source_updated_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET category_id=excluded.category_id,sku=excluded.sku,name=excluded.name,company=excluded.company,description=excluded.description,unit_of_measure=excluded.unit_of_measure,tax_code=excluded.tax_code,gst_rate_bps=excluded.gst_rate_bps,mrp=excluded.mrp,expiry_date=excluded.expiry_date,is_weight_based=excluded.is_weight_based,is_active=excluded.is_active,allow_manual_price=excluded.allow_manual_price,track_inventory=excluded.track_inventory,version=excluded.version,source_updated_at=excluded.source_updated_at,updated_at=excluded.updated_at WHERE excluded.version>=catalog_products.version`, p.ID, p.CategoryID, p.SKU, p.Name, p.Company, p.Description, p.UnitOfMeasure, p.TaxCode, gstRateBps, p.MRP, p.ExpiryDate, boolInt(p.IsWeightBased), boolInt(p.IsActive), boolInt(p.AllowManualPrice), boolInt(p.TrackInventory), p.Version, p.SourceUpdatedAt, now)
	if err != nil {
		return err
	}
	if p.TrackInventory && p.StoreID != nil && *p.StoreID != "" && p.StockQuantity != nil {
		onHandMilli := int64(math.Round(*p.StockQuantity * 1000))
		_, err = tx.ExecContext(ctx, `INSERT INTO inventory_balances(store_id,product_id,on_hand_milli,reserved_milli,version,updated_at) VALUES(?,?,?,?,?,?) ON CONFLICT(store_id,product_id) DO UPDATE SET on_hand_milli=excluded.on_hand_milli,version=excluded.version,updated_at=excluded.updated_at WHERE excluded.version>=inventory_balances.version`, *p.StoreID, p.ID, onHandMilli, 0, p.Version, now)
	}
	return err
}

type productRemovePayload struct {
	ID              string  `json:"id"`
	Version         int     `json:"version"`
	SourceUpdatedAt *string `json:"source_updated_at"`
}
...
func removeProduct(ctx context.Context, tx *sql.Tx, raw []byte) error {
	...
	if json.Unmarshal(raw, &p) != nil || p.ID == "" || p.Version <= 0 {
		return errors.New("invalid_product_remove_payload")
	}
	now := time.Now().UTC().Format(time.RFC3339Nano)
	...
	if _, err = tx.ExecContext(ctx, `DELETE FROM catalog_barcodes WHERE product_id=?`, p.ID); err != nil {
		return err
	}
	if _, err = tx.ExecContext(ctx, `DELETE FROM catalog_prices WHERE product_id=?`, p.ID); err != nil {
		return err
	}
	return nil
}
...
func upsertBarcode(ctx context.Context, tx *sql.Tx, raw []byte) error {
	...
	if json.Unmarshal(raw, &p) != nil || p.Barcode == "" || p.ProductID == "" {
		return errors.New("invalid_barcode_payload")
	}
	now := time.Now().UTC().Format(time.RFC3339Nano)
	if p.IsPrimary {
		if _, err := tx.ExecContext(ctx, `DELETE FROM catalog_barcodes WHERE product_id=? AND is_primary=1 AND barcode<>?`, p.ProductID, p.Barcode); err != nil {
			return err
		}
	}
...
type customerMappingPayload struct {
	ID            string `json:"id"`
...
type customerPayload struct {
	...
	POSMappings      []customerMappingPayload `json:"pos_mappings"`
...
func resolveCustomerTarget(ctx context.Context, tx *sql.Tx, p customerPayload) (string, bool, error) {
	...
	for _, mapping := range p.POSMappings {
		if mapping.ID == "" || mapping.SourceVersion < 0 {
			return "", false, errors.New("invalid_customer_mapping_payload")
		}
		...
		if targetID != "" && targetID != mapping.ID {
			return "", false, errors.New("ambiguous_customer_mapping")
		}
		targetID = mapping.ID
		if (state == "pending" || state == "conflict") && localVersion > mapping.SourceVersion {
			preservePending = true
		}
	}
	if targetID != "" {
		return targetID, preservePending, nil
	}
...
func upsertCustomer(ctx context.Context, tx *sql.Tx, raw []byte) error {
	...
	if json.Unmarshal(raw, &p) != nil || p.ID == "" || p.Name == "" {
		return errors.New("invalid_customer_payload")
	}
