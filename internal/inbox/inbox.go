...
func upsertBarcode(ctx context.Context,tx *sql.Tx,raw []byte) error { var p barcodePayload; if json.Unmarshal(raw,&p)!=nil || p.Barcode=="" || p.ProductID=="" { return errors.New("invalid_barcode_payload") }; now:=time.Now().UTC().Format(time.RFC3339Nano); _,err:=tx.ExecContext(ctx,`INSERT INTO catalog_barcodes(barcode,product_id,barcode_type,is_primary,updated_at) VALUES(?,?,?,?,?) ON CONFLICT(barcode) DO UPDATE SET product_id=excluded.product_id,barcode_type=excluded.barcode_type,is_primary=excluded.is_primary,updated_at=excluded.updated_at`,p.Barcode,p.ProductID,p.BarcodeType,boolInt(p.IsPrimary),now); return err }
