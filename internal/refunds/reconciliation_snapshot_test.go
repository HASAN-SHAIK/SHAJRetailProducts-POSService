...
func TestGetReconciliationSnapshotIncludesRefundSyncState(t *testing.T) {
	...
	for _, event := range []struct {
		...
		{id: "evt-reconciliation-published", status: "published"},
		{id: "evt-reconciliation-pending", status: "pending"},
		{id: "evt-reconciliation-dead", status: "dead_letter"},
	} {
		if _, err := db.SQL().ExecContext(ctx, `
			...
			event.id, "sales_order", "ord-refund-full", 3, "sale.partial_returned", 1, "sales_order:ord-refund-full",
