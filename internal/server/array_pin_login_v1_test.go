...
func TestLocalAuthLoginRejectsArrayPINMember(t *testing.T) {
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected array pin member to be rejected with 400, got status=%d body=%s", rec.Code, rec.Body.String())
	}
