...
func TestLocalAuthLoginRejectsWhitespaceOnlyBody(t *testing.T) {
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected whitespace-only login body to return 400, got status=%d body=%s", rec.Code, rec.Body.String())
	}
