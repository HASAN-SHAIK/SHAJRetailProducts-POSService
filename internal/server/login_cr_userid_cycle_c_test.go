...
func TestLocalAuthLoginRejectsCRUserIDMember(t *testing.T) {
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected CR user_id member to be rejected with 400, got status=%d body=%s", rec.Code, rec.Body.String())
	}
