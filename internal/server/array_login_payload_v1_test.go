...
func TestLocalAuthLoginRejectsJSONArrayPayload(t *testing.T) {
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected JSON array login payload to be rejected with 400, got status=%d body=%s", rec.Code, rec.Body.String())
	}
