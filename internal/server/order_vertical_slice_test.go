...
func TestOrderVerticalSlicePersistsAcrossSQLiteRestart(t *testing.T) {
	...
	var onHand int64
	if err := reopened.SQL().QueryRow(`SELECT on_hand_milli FROM inventory_balances WHERE store_id='store-1' AND product_id='product-1'`).Scan(&onHand); err != nil { t.Fatal(err) }
	if onHand != -1000 { t.Fatalf("on_hand_milli=%d", onHand) }

	snap, err := observability.New(reopened, outbox.New(reopened), filepath.Join(t.TempDir(), "backups")).Collect(ctx)
	if err != nil { t.Fatalf("collect observability: %v", err) }
	if !snap.DatabaseOK || snap.Outbox.Pending != 2 { t.Fatalf("unexpected observability snapshot: %#v", snap) }
}
...
func openMigratedDB(t *testing.T, path string) *database.DB {
	...
	if err := db.Migrate(context.Background()); err != nil { db.Close(); t.Fatalf("migrate database: %v", err) }
	if err := db.IntegrityCheck(context.Background()); err != nil { db.Close(); t.Fatalf("integrity check: %v", err) }
	return db
}

func newTestServer(db *database.DB, deviceService *device.Service) *Server {
	catalogRepo := catalog.NewRepository(db)
	return New(config.Config{Environment: "test", ListenAddress: "127.0.0.1:0"}, db, deviceService, catalogRepo,
		customer.NewRepository(db), orders.New(db, catalogRepo), payments.New(db), inventory.New(db), receipts.New(db))
}
...
func postOrder(t *testing.T, app *Server) string {
	t.Helper()
	body := map[string]any{"client_order_id": "client-order-vertical-1", "currency": "INR", "items": []map[string]any{{"product_id": "product-1", "quantity_milli": 1000, "discount_minor": 0, "tax_minor": 0}}}
	res := serveJSON(t, app, http.MethodPost, "/api/v1/orders", body)
	if res.Code != http.StatusCreated { t.Fatalf("create order status=%d body=%s", res.Code, res.Body.String()) }
	var order orders.Order
	if err := json.NewDecoder(res.Body).Decode(&order); err != nil { t.Fatal(err) }
	if order.ID == "" || len(order.Items) != 1 { t.Fatalf("unexpected order: %#v", order) }
	return order.ID
}
...
func postPayment(t *testing.T, app *Server, orderID, clientPaymentID string) {
	...
	if res.Code != http.StatusCreated { t.Fatalf("create payment status=%d body=%s", res.Code, res.Body.String()) }
}
...
func completeOrder(t *testing.T, app *Server, orderID string) {
	...
	if res.Code != http.StatusOK { t.Fatalf("complete order status=%d body=%s", res.Code, res.Body.String()) }
}

func serveJSON(t *testing.T, app *Server, method, target string, payload any) *httptest.ResponseRecorder {
	t.Helper()
	var body bytes.Buffer
	if payload != nil { if err := json.NewEncoder(&body).Encode(payload); err != nil { t.Fatal(err) } }
