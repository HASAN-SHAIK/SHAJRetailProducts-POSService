...
func TestLocalAuthLoginRejectsCRPINMember(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/auth/login", strings.NewReader(`{"user_id":"cashier","pin":"\u000d"}`))
	rec := httptest.NewRecorder()

	s.handleLocalAuthLogin(rec, req)

	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected CR pin member to be rejected with 400, got status=%d body=%s", rec.Code, rec.Body.String())
	}
