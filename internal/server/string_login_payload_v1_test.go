...
func TestLocalAuthLoginRejectsJSONStringPayload(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/auth/login", strings.NewReader(`"cycle-c-login"`))
	rec := httptest.NewRecorder()

	s.handleLocalAuthLogin(rec, req)

	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected JSON string login payload to be rejected with 400, got status=%d body=%s", rec.Code, rec.Body.String())
	}
