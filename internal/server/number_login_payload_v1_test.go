...
func TestLocalAuthLoginRejectsJSONNumberPayload(t *testing.T) {
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected JSON number login payload to be rejected with 400, got status=%d body=%s", rec.Code, rec.Body.String())
	}
