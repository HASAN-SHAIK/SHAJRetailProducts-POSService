...
func TestLocalAuthLoginRejectsWhitespaceOnlyUserIDMember(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/auth/login", strings.NewReader(`{"user_id":"   ","pin":"2468"}`))
	rec := httptest.NewRecorder()

	s.handleLocalAuthLogin(rec, req)

	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected whitespace-only user_id member to be rejected with 400, got status=%d body=%s", rec.Code, rec.Body.String())
	}
