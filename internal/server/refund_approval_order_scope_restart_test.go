...
func TestRefundApprovalWrongOrderRemainsUnconsumedAcrossRestart(t *testing.T) {
	...
	if _, err := (&Server{db: db}).consumeManagerApprovalForOrder(ctx, token, cashierID, permissionPOSRefund, attemptedID); err == nil {
		db.Close()
	...
	approval, err := (&Server{db: db}).consumeManagerApprovalForOrder(ctx, token, cashierID, permissionPOSRefund, approvedID)
	if err != nil {
		t.Fatalf("correct order could not consume preserved approval after restart: %v", err)
	}
	...
	if _, err := (&Server{db: db}).consumeManagerApprovalForOrder(ctx, token, cashierID, permissionPOSRefund, approvedID); err == nil {
