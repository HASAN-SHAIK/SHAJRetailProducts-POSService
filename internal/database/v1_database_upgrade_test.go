...
func TestV1RepresentativePreviousSchemaUpgradePreservesDurableFacts(t *testing.T) {
	...
	// Build the immediately previous embedded schema exactly as an older POS
	// installation would have seen it, including the historical migration ledger.
	...
	// Simulate installation of the current POS binary: reopen the existing file
	// and let the production migration engine apply only the remaining migration.
