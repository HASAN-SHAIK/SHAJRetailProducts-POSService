...
func TestRealCentralRefundE2E(t *testing.T) {
	...
	if _, err := db.SQL().ExecContext(ctx, `
		...
		VALUES('product-refund-e2e','Refund E2E Product','unit',1,0,1,1,?)`, now); err != nil {
		t.Fatal(err)
	}
	if _, err := db.SQL().ExecContext(ctx, `
		...
		VALUES(?,?,?,?,'paid','INR',10000,0,0,10000,'pos',2,?,?,?)`,
		orderID, "client-refund-cross-repo-e2e", "store-e2e", "terminal-e2e", now, now, now); err != nil {
		t.Fatal(err)
	}
	if _, err := db.SQL().ExecContext(ctx, `
		...
		VALUES(?,?,1,'product-refund-e2e','Refund E2E Product',1000,10000,0,0,10000,?)`, itemID, orderID, now); err != nil {
		t.Fatal(err)
	}
	if _, err := db.SQL().ExecContext(ctx, `
		INSERT INTO inventory_balances(store_id,product_id,on_hand_milli,reserved_milli,version,updated_at)
		VALUES('store-e2e','product-refund-e2e',4000,0,2,?)`, now); err != nil {
		t.Fatal(err)
	}
	if _, err := db.SQL().ExecContext(ctx, `
		...
		VALUES('issue-refund-cross-repo-e2e','store-e2e','product-refund-e2e','sale_issue',-1000,
			'sale_order',?,?,4000,?,?)`, orderID, itemID, now, now); err != nil {
		t.Fatal(err)
	}

	paymentService := payments.New(db)
	capture, _, err := paymentService.Create(ctx, orderID, payments.CreateInput{
		ClientPaymentID: "capture-refund-cross-repo-e2e",
		Mode:            "cash",
		AmountMinor:     10000,
		Currency:        "INR",
		Status:          "captured",
	...
	// sale.completed carries the original captured tender. Keep the standalone
	// capture event out of this test so the refund assertions isolate the
	// compensating payment event produced by RefundFullSale.
	...
	completedPayload := map[string]any{
		"order": map[string]any{
			"id": orderID,
			"client_order_id": "client-refund-cross-repo-e2e",
			"store_id": "store-e2e",
			...
			"items": []map[string]any{{
				"id": itemID,
				"line_no": 1,
				"product_id": "product-refund-e2e",
				"product_name": "Refund E2E Product",
				"quantity_milli": 1000,
		...
		"payments": []map[string]any{{
			"id": capture.ID,
			"client_payment_id": capture.ClientPaymentID,
			"mode": capture.Mode,
			"direction": "in",
			"amount_minor": 10000,
			"currency": "INR",
		...
		"inventory_movements": []map[string]any{{
			"id": "issue-refund-cross-repo-e2e",
			"store_id": "store-e2e",
			"product_id": "product-refund-e2e",
	...
	if _, err := db.SQL().ExecContext(ctx, `INSERT INTO outbox_events(
		...
		VALUES(?,?,?,?,?,?,?,?,?,'pending',0,?,?)`,
		completedEventID, "sales_order", orderID, 2, "sale.completed", 1, "sales_order:"+orderID,
	...
	if returned.Status != "returned" || returned.Version != 3 {
		t.Fatalf("returned order status/version=%s/%d", returned.Status, returned.Version)
	}

	var outbound, saleReturns int
	var onHand int64
	if err := db.SQL().QueryRowContext(ctx, `SELECT COUNT(*) FROM payments WHERE order_id=? AND direction='out' AND status='refunded' AND amount_minor=10000`, orderID).Scan(&outbound); err != nil {
		t.Fatal(err)
	}
	if err := db.SQL().QueryRowContext(ctx, `SELECT COUNT(*) FROM inventory_movements WHERE order_item_id=? AND movement_type='sale_return'`, itemID).Scan(&saleReturns); err != nil {
		t.Fatal(err)
	}
	if err := db.SQL().QueryRowContext(ctx, `SELECT on_hand_milli FROM inventory_balances WHERE store_id='store-e2e' AND product_id='product-refund-e2e'`).Scan(&onHand); err != nil {
		t.Fatal(err)
	}
	if outbound != 1 || saleReturns != 1 || onHand != 5000 {
		t.Fatalf("local refund compensation outbound=%d sale_returns=%d on_hand=%d", outbound, saleReturns, onHand)
	}

	// Drain the actual refund-produced payment, inventory and sale.returned facts.
	for i := 0; i < 10; i++ {
		var pending int
		...
		if !engine.dispatchOne(ctx) {
			t.Fatal("refund outbox remained pending but dispatch made no progress")
		}
	}
	...
	if pending != 0 {
		t.Fatalf("refund left %d pending outbox events", pending)
	}

	var returnedEventID string
	if err := db.SQL().QueryRowContext(ctx, `SELECT id FROM outbox_events WHERE aggregate_id=? AND event_type='sale.returned'`, orderID).Scan(&returnedEventID); err != nil {
		t.Fatal(err)
	}
	assertRealCentralOutboxState(t, db, returnedEventID, "published")

	// Replay the exact durable return fact. Central source-version/idempotency
	// handling must preserve one canonical returned sale and its audit metadata.
	if _, err := db.SQL().ExecContext(ctx, `UPDATE outbox_events SET status='pending',published_at=NULL,locked_at=NULL,last_error=NULL,available_at=? WHERE id=?`,
		time.Now().UTC().Format(time.RFC3339Nano), returnedEventID); err != nil {
		t.Fatal(err)
	}
	if !engine.dispatchOne(ctx) {
		t.Fatal("expected sale.returned replay dispatch")
	}
	assertRealCentralOutboxState(t, db, returnedEventID, "published")
}
