...
func TestRefundOrderingKeyBlocksLaterFactsUntilLostAckEventRecovers(t *testing.T) {
	...
	central := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if key == events[0].id && requestNumber == 1 {
			// Central commits the earliest refund fact, but POS never receives its
			// acknowledgement. The shared ordering key must prevent later facts
			// from overtaking it while its local outbox row remains retryable.
	...
	// The earliest row is deliberately in retry backoff. A second dispatch must
	// not skip ahead to another pending row with the same ordering key.
	if engine.dispatchOne(ctx) {
