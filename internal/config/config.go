...
func Load() (Config, error) {
	...
	if cfg.CentralAPIURL != "" && (cfg.CentralTenantID == "" || cfg.CentralSyncToken == "") { return Config{}, fmt.Errorf("POS_SYNC_TENANT_ID and POS_SYNC_TOKEN are required when POS_CENTRAL_API_URL is configured") }
...
func validateLoopbackAddress(address string) error {
	...
	if err != nil { return fmt.Errorf("POS_LISTEN_ADDRESS: %w", err) }
	if strings.EqualFold(host, "localhost") { return nil }
	ip := net.ParseIP(host)
	if ip == nil || !ip.IsLoopback() { return fmt.Errorf("POS_LISTEN_ADDRESS must bind to loopback only") }
	return nil
}
...
func warnDeprecatedEnv(oldKey, newKey string) { slog.Warn("deprecated environment variable configured", "old", oldKey, "new", newKey) }
