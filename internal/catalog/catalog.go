...
func (r *Repository) GetProduct(ctx context.Context, id, storeID string) (Product, error) {
    ...
    if err != nil {
        if errors.Is(err, sql.ErrNoRows) { return Product{}, ErrNotFound }
        return Product{}, fmt.Errorf("get catalog product: %w", err)
    }
    if err := r.attachBarcodesAndPrice(ctx, &p, storeID); err != nil { return Product{}, err }
    return p, nil
}

func (r *Repository) GetByBarcode(ctx context.Context, barcode, storeID string) (Product, error) {
    barcode = strings.TrimSpace(barcode)
    if barcode == "" { return Product{}, ErrNotFound }
    row := r.db.SQL().QueryRowContext(ctx, `
...
func (r *Repository) Search(ctx context.Context, query, storeID string, limit int) ([]Product, error) {
    ...
    if err != nil { return nil, fmt.Errorf("search catalog: %w", err) }
    defer rows.Close()
    var out []Product
    for rows.Next() {
        p, err := scanProduct(rows)
        if err != nil { return nil, err }
        if err := r.attachBarcodesAndPrice(ctx, &p, storeID); err != nil { return nil, err }
        out = append(out, p)
    }
    return out, rows.Err()
}
...
func (r *Repository) ListCategories(ctx context.Context) ([]Category, error) {
    ...
    for rows.Next() {
        var c Category
        if err := rows.Scan(&c.ID, &c.ParentID, &c.Name, &c.Code, &c.SortOrder); err != nil { return nil, err }
        out = append(out, c)
    }
    return out, rows.Err()
}
...
func (r *Repository) attachBarcodesAndPrice(ctx context.Context, p *Product, storeID string) error {
    ...
    if err != nil { return fmt.Errorf("load barcodes: %w", err) }
    for rows.Next() {
        var code string
        if err := rows.Scan(&code); err != nil { rows.Close(); return err }
    ...
    if err != nil && !errors.Is(err, sql.ErrNoRows) { return fmt.Errorf("load price: %w", err) }
    if err == nil {
