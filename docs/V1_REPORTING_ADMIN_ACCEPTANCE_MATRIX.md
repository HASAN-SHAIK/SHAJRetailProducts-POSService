...
|---|---|---|---|
| Central report permission boundary | Backend #76 aligns all five core report routes with `reports:read`; Backend #84 extends the same permission boundary to mobile reporting; cashier remains denied by the certified role catalog | CERTIFIED | preserve stable server-side permission denial and trusted Central report scope |
...
| Inventory report | Central reports total stock, low/out-of-stock, inventory value and cost value from canonical products; Backend #80 deliberately fails closed for branch-scoped inventory reports because the legacy tenant-level product stock projection is not branch truth; Backend #84 also suppresses/fails closed mobile branch inventory facts | PARTIAL | establish the certified branch inventory reporting source, batch-aware consistency expectations and focused tenant/branch acceptance |
| Dashboard/admin KPI views | Frontend Dashboard and Central reporting/dashboard infrastructure already exist; Backend #84 now scopes mobile dashboard sales/profit/recent-order facts to trusted branch authority and avoids tenant stock leakage; Frontend #73 makes the routed mobile Reports screen truthful and retryable | PARTIAL | audit the remaining routed dashboard KPI APIs, remove caller-controlled branch/browser-cache authority, and add loading/error/empty/retry acceptance |
...
| Date/range validation | Backend #82 certifies strict UTC daily `YYYY-MM-DD` validation; Frontend #73 removes fake custom ranges from the fixed-window mobile report screen; broader range-based Central report paths still lack one bounded V1 contract | PARTIAL | establish one V1 date-range contract, invalid-range behavior and bounded query limits across range-based reports |
| Large-result safety | several detail lists are bounded, including mobile low-stock pagination, but Reporting V1 has no unified pagination/range matrix yet | GAP | certify bounded result sizes / date windows and avoid unbounded administrative queries |
...
- Backend #84: mobile reporting now requires `reports:read`, uses the certified report scope, scopes mobile sales/profit/recent-order facts to the trusted branch, never falls back to a global DB pool, and refuses to leak tenant-level stock into a branch report while branch inventory truth remains unresolved.
- Frontend #73: mobile report UX now labels only the fixed Today/Week/Month windows it actually receives, exposes unavailable data as unavailable rather than zero, and provides an explicit retry state with production-build acceptance.
...
2. Close trusted scope/permission/inventory-truth gaps in the remaining Dashboard/Admin KPI routes.
3. Establish one bounded V1 date/range contract and certify invalid-range behavior.
4. Certify product-history, tax/GST and customer/outstanding reporting against their already-certified canonical snapshots.
