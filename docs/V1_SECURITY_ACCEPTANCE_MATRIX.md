...
| Dependency/supply-chain review | POS Go dependency is checksum-pinned; Frontend #78 now builds its production container on Node 24 with committed lockfile + `npm ci`; Backend still has no npm lockfile and Frontend still declares `jsonwebtoken` using the floating `latest` spec | PARTIAL | establish reproducible Backend dependency resolution, eliminate remaining floating production specs, audit current advisories/runtime reachability and document accepted residuals |
...
1. Establish remaining dependency reproducibility: Backend lockfile policy and Frontend floating dependency specs/advisory triage.
...
**NOT YET RELEASE CERTIFIED.** Core tenant/store/device/token/session/recovery security is strongly covered; auth brute-force, browser cookie/CORS, bounded import/upload admission, production RabbitMQ credential safety and Frontend lockfile-enforced container builds now have executable evidence. Remaining V1 Security work is concentrated on Backend dependency reproducibility, Frontend floating/advisory dependency review, the remaining input/query/config audit and the final consolidated release gate.
