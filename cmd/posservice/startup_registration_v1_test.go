...
func TestV1POSNumberOnlyConfiguredRegistrationFailsClosedBeforeListener(t *testing.T) {
	t.Parallel()

	tempDir := t.TempDir()
	dbPath := filepath.Join(tempDir, "pos-number-only-registration.db")
	...
	if runErr == nil {
		t.Fatalf("expected POS-number-only registration to prevent POSService startup; output=%s", output)
	}
	...
	if dialErr == nil {
		_ = conn.Close()
		t.Fatalf("POSService listener became reachable despite POS-number-only registration at %s", listenAddress)
	}
	...
	if storeID.Valid || storeNumber.Valid || posNo.Valid || touchpointID.Valid {
		t.Fatalf("POS-number-only configured registration leaked into SQLite: store_id=%q store_number=%q pos_no=%q touchpoint_id=%q",
			storeID.String, storeNumber.String, posNo.String, touchpointID.String)
	}
}
