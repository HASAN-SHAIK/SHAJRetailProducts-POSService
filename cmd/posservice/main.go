...
func main() {
	...
	identity, err := deviceService.EnsureInstallationWithSeed(startupCtx, device.InstallationSeed{DeviceID: cfg.DeviceID, InstallationID: cfg.InstallationID})
	if err != nil {
	...
	if err != nil {
		slog.Error("initialize local API security", "error", err)
		os.Exit(1)
	}
	...
	orderService.SetDiscountPolicy(func(ctx context.Context) (orders.DiscountPolicy, error) {
		...
		if err != nil {
			return orders.DiscountPolicy{}, err
		}
		...
		if maxPercent < 0 || maxPercent > 100 {
			return orders.DiscountPolicy{}, fmt.Errorf("billing.max_discount_percent must be between 0 and 100")
		}
	...
	syncEngine, err := syncengine.New(eventOutbox, cfg.CentralAPIURL, cfg.CentralTenantID, cfg.CentralSyncToken, identity.DeviceID, cfg.SyncRequestTimeout, cfg.SyncPollInterval)
	if err != nil {
		slog.Error("configure sync engine", "error", err)
		os.Exit(1)
	}
	...
	if cfg.CentralAPIURL != "" {
		...
		if configErr != nil {
			slog.Error("configure effective configuration sync", "error", configErr)
			os.Exit(1)
		}
	...
	if changePuller != nil {
		go changePuller.Run(ctx)
	}
	if effectiveConfigService != nil && effectiveConfigService.Enabled() {
		go effectiveConfigService.Run(ctx)
	}
