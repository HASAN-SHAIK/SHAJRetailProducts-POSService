...
bin=${1:?usage: v1-login-null-payload-runtime.sh /path/to/posservice}
work=/tmp/shaj-cycle-c-login-null-payload
rm -rf "$work"
mkdir -p "$work/state" "$work/backups"
local_token=cycle-c-login-null-payload-token-1234567890

export POS_ENVIRONMENT=development
export POS_LISTEN_ADDRESS=127.0.0.1:4807
...
for i in $(seq 1 40); do
  if curl --silent --fail --max-time 1 http://127.0.0.1:4807/api/v1/health > "$work/health-before.json" 2>/dev/null; then
    ready=true
    break
  fi
...
echo "NULL_LOGIN_INITIAL_HEALTH=$ready"
if [[ "$ready" != true ]]; then
  echo 'NULL_LOGIN_RUNTIME_PASS=false'
...
status=$(curl --silent --show-error --max-time 5 \
  ...
  --data-binary 'null' \
  http://127.0.0.1:4807/api/v1/auth/login || true)
body=$(cat "$work/response.json" 2>/dev/null || true)

process_alive=false
if kill -0 "$pid" 2>/dev/null; then process_alive=true; fi
after=$(curl --silent --output "$work/health-after.json" --write-out '%{http_code}' --max-time 2 http://127.0.0.1:4807/api/v1/health || true)

sqlite_ok=false
if [[ -s "$work/state/pos.db" ]]; then sqlite_ok=true; fi

echo "NULL_LOGIN_RESPONSE_STATUS=$status"
echo "NULL_LOGIN_RESPONSE_BODY=$body"
echo "NULL_LOGIN_PROCESS_ALIVE=$process_alive"
echo "NULL_LOGIN_HEALTH_AFTER=$after"
echo "NULL_LOGIN_SQLITE_PRESENT=$sqlite_ok"
echo '--- POS log ---'
cat "$work/pos.log"

if [[ "$status" == "400" && "$body" == *'invalid_auth_payload'* && "$process_alive" == "true" && "$after" == "200" && "$sqlite_ok" == "true" ]]; then
  echo 'NULL_LOGIN_RUNTIME_PASS=true'
  exit 0
fi
